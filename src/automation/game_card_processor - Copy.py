...
class GameCardProcessor:
    ...
    def load_enrollment_data(self, file_path: str = None) -> pd.DataFrame:
        ...
        active_statuses = ['Paid', 'Partial', 'Active']
        if 'Order Payment Status' in df_upper.columns:
            df_upper = df_upper[df_upper['Order Payment Status'].isin(active_statuses)]
        
        logger.info(f"Found {len(df_upper)} players in upper divisions")
    ...
    def _process_division(self, workbook: openpyxl.Workbook, 
        ...
        div_df = div_df.sort_values(['Team Name', 'Jersey Number'], na_position='last')
    ...
    def _matches_target_division(self, division_name: str, target_division: str) -> bool:
       ...
       """
       return target_division in division_name
    ...
    def _add_team_section(self, sheet, team_df: pd.DataFrame, 
        ...
        for _, player in team_df.iterrows():
            sheet[f'A{current_row}'] = player.get('Jersey Number', '')
        ...
        if 'Standard Division' in enrollment_df.columns:
            for division in self.upper_divisions:
                ...
                div_summary = {
                    ...
                    'players_with_jerseys': div_df['Jersey Number'].notna().sum(),

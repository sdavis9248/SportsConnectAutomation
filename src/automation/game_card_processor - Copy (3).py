...
class GameCardProcessor:
    ...
    def process_game_card(self, 
        ...
        try:
            ...
            # Load the game card workbook (THIS IS WHERE THE TEMPLATE IS OPENED)
    ...
    def _process_division(self, workbook: openpyxl.Workbook, 
        ...
        # Check if we should create a new sheet or use existing one
        sheet_name = f"Game Card {division}"
        
        # If the sheet already exists, use it; otherwise use the original Game Card sheet
        if sheet_name in workbook.sheetnames:
            sheet = workbook[sheet_name]
            logger.info(f"Using existing sheet: {sheet_name}")
        elif "Game Card" in workbook.sheetnames:
            # For the first division, we can use the original Game Card sheet
            # and rename it
            if division == self.upper_divisions[0]:
                sheet = workbook["Game Card"]
                sheet.title = sheet_name
                logger.info(f"Renamed 'Game Card' sheet to: {sheet_name}")
            else:
                # For subsequent divisions, we need to handle the graphics issue
                # Option 1: Alert the user to manually duplicate the sheet
                logger.warning(f"Cannot automatically copy sheet with graphics. Please manually duplicate 'Game Card' sheet and rename to '{sheet_name}'")
                return
                
                # Option 2: Create a new sheet without graphics (uncomment if preferred)
                # sheet = workbook.create_sheet(sheet_name)
                # logger.info(f"Created new sheet (without graphics): {sheet_name}")
        else:
            logger.error("Template sheet 'Game Card' not found in workbook")
            return
    ...
    def _fill_game_card_team(self, sheet, team_df: pd.DataFrame, division: str, team_name: str, position: int):
        ...
        # You'll need to adjust these based on the actual template
        team_sections = {
            1: {
                'division': 'F4',  # Adjust based on actual template
                'team_name': 'F5',
                'coach': 'D7',
                'assistant': 'G7',
                'team_id': 'J4',
            ...
            2: {
                'division': 'S4',  # Adjust based on actual template
                'team_name': 'S5',
                'coach': 'Q7',
                'assistant': 'T7',
                'team_id': 'W4',
            ...
            3: {
                'division': 'AF4',  # Adjust based on actual template
                'team_name': 'AF5',
                'coach': 'AD7',
                'assistant': 'AG7',
                'team_id': 'AJ4',
        ...
        for _, player in team_df.iterrows():
            ...
            if current_row > section['player_start_row'] + 19:  # Adjust based on template
